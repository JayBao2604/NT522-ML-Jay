...
def get_fitness_score(pre_result_path, adv_file_path, snippet_len, penalty):
    """
    Tính điểm fitness: 1 - accuracy - penalty * snippet_length
    Đầu vào: file predict.txt (dự đoán), file CSV thật (ground-truth)
    """

    # Đọc dự đoán
    pre_dic = {}
    with open(pre_result_path) as f:
        for line in f:
            if line.strip():
                idx, label = line.strip().split('\t')
                pre_dic[int(idx)] = int(label)

    # Đọc nhãn thật từ CSV
    df = pd.read_csv(adv_file_path)
    test_data_dic = dict(zip(df["idx"], df["target"]))

    # So sánh trên các chỉ số có trong cả hai
    common_ids = set(pre_dic.keys()) & set(test_data_dic.keys())
    if not common_ids:
        return 1.0  # Nếu không có mẫu chung, trả về điểm kém

    pre_list = [pre_dic[i] for i in common_ids]
    true_list = [test_data_dic[i] for i in common_ids]
...
def select(pop_dict, centroid, centriod_array, decay_rate):
    fitness_values = []
    keys_list = []
    ...
    f_sum = np.sum(factor)
    p = np.array([element / f_sum for element in factor], dtype=np.float64)
...
def get_vul_codes(test_dicts, vul_idx):
    ...
    for i in test_dicts:
       if i['idx'] in vul_idx:
         vul_codes[i['idx']] = i['processed_func']
    return vul_codes
