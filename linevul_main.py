...
from transformers import AutoTokenizer, AutoModel, T5Tokenizer, T5ForSequenceClassification, T5Config, AutoModelForSeq2SeqLM
from peft import PeftConfig, PeftModel
from tqdm import tqdm
import multiprocessing
from linevul_model import Model
# from linevul_codeT5 import Model
import pandas as pd
# metrics
    ...
    if do_explanation:
        if reasoning_method == "attention":
            ...
            if reasoning_method == "deeplift":
                ...
                baselines = torch.zeros(1, 64, 768, requires_grad=True).to(args.device)
                ...
                baselines = torch.zeros(16, 64, 768, requires_grad=True).to(args.device)
...
def line_level_localization(flaw_lines: str, tokenizer, model, mini_batch, original_func: str, args,
    ...
    if reasoning_method == "attention":
        ...
        if reasoning_method == "deeplift":
            ...
            #baselines = torch.randn(16, 64, 768, requires_grad=True).to(args.device)
            baselines = torch.zeros(16, 64, 768, requires_grad=True).to(args.device)
            reasoning_model = DeepLiftShap(model)
        elif reasoning_method == "gradient_shap":
            #baselines = torch.randn(16, 512, 768, requires_grad=True).to(args.device)
...
def main():
    ...
    parser.add_argument("--test_data_file", default=None, type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--model_name", default="model_02052025.bin", type=str,
    ...
    # config = PeftConfig.from_pretrained(args.model_name_or_path, task_type='SEQ_2_SEQ_LM')
    config.num_labels = 1
    ...
    if args.use_word_level_tokenizer:
        ...
        # tokenizer = AutoTokenizer(vocab_file="FTGraphCodeBert.pt/vocab.json",
        #                           merges_file="FTGraphCodeBert.pt/merges.txt")
    else:
    ...
    if args.use_non_pretrained_model:
        ...
        # model_1 = AutoModelForSeq2SeqLM.from_pretrained(config)
        ...
        # model=PeftModel.from_pretrained(model_1, args.model_name_or_path, ignore_mismatched_sizes=True)    
